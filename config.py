...
EMBEDDING_CONFIG = {
    "base_url": os.getenv("EMBED_BASE_URL", "https://api.ephone.ai/v1"),
...
# ============ 成本监控 ============
def _cost(
    model: str, input_key: str, output_key: str, default_in: float, default_out: float
) -> dict:
    return {
...
COST_CONFIG = {
    ...
    "deepseek-v4-flash": _cost(
        "v4flash", "COST_SECRETARY_INPUT", "COST_SECRETARY_OUTPUT", 1.0, 2.0
    ),
    "deepseek-v4-flash-vision-exp": _cost(
        "v4flashvision", "COST_DECISION_INPUT", "COST_DECISION_OUTPUT", 1.5, 4.5
    ),
    "deepseek-v4-pro": {"input": 3.0, "output": 6.0},
    "text-embedding-3-small": _cost(
        "embed3small", "COST_EMBED_INPUT", "COST_EMBED_OUTPUT", 0.02, 0.0
    ),
