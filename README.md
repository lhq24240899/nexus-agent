...
| 功能模块 | 协作方式 |
|---|---|
| 双核架构（决策核心 + 秘书核心） | Doubao 协助设计架构、编写核心逻辑 |
| 四库管理 + 向量检索 | Doubao 协助实现存储、索引、检索 |
| 嵌入 Linux（Docker/WSL2/Mock） | Doubao 协助实现三种模式自动检测 |
| Web 控制台（对话/日志/终端/四库/成本/Stats） | Doubao 协助编写 Flask 后端 + 前端 |
| 工具系统（20+ 工具 + MCP + 插件） | Doubao 协助实现工具注册、执行、安全沙箱 |
| 技能系统（自我改进 + 置信度 + 版本回滚） | Doubao 协助实现技能匹配、改进、回滚机制 |
| 上下文压缩（80% 阈值 + LLM 摘要） | Doubao 协助实现三层防护 |
| 临时工作目录 + 自动清理 | Doubao 协助实现任务级隔离 + 多维度清理 |
| SSE 流式输出（心跳 + 断线续传） | Doubao 协助实现双线程队列 + 事件 ID |
| 任务统计 + 成本监控 | Doubao 协助实现 SQLite 持久化 + 前端可视化 |
| 并行执行 + 工具自动重试 | Doubao 协助实现子代理线程池 + 瞬时错误重试 |

人类开发者负责：需求定义、架构决策、代码审查、功能验证、Git 管理。
