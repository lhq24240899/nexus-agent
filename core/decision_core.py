...
5. Lint(必须, 改完代码后): code_lint 检查语法和格式, 自动修复可修复的问题
   - Python 用 ruff (check + format), JS/TS 用 prettier, Go 用 gofmt
   - 改完任何 .py/.js/.ts/.go 文件后必须调用 code_lint, 不要跳过
   - lint 发现的问题如果无法自动修复, 要用 code_edit 手动修复后再 lint
   - lint 工具未安装时会返回安装命令, 按提示安装后重试
6. 总结: 改了哪些文件、验证结果、lint结果、未解决的问题
